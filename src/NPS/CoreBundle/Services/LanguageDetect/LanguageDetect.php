...
namespace NPS\CoreBundle\Services\LanguageDetect;
...
class LanguageDetect
{
     ...
     * @access     private
     * @deprecated Superceded by the Parser class
     */
    ...
    {
        // input check
        if (!Parser::validateString($sample)) {
            return array();
        }
    ...
    function clusterLanguages()
    {
        ...
        foreach ($langs as $lang) {
            if (!isset($this->_lang_db[$lang])) {
                throw new TextException(
                    "missing $lang!",
                    TextException::UNKNOWN_LANGUAGE
                );
            }
        }
    ...
    function _convertFromNameMode($lang, $convertKey = false)
    {
        ...
        if (is_string($lang)) {
            return (string)ISO639::$method($lang);
        }

        $newlang = array();
        foreach ($lang as $key => $val) {
            if ($convertKey) {
                $newkey = (string)ISO639::$method($key);
                $newlang[$newkey] = $val;
            } else {
                $newlang[$key] = (string)ISO639::$method($val);
            }
        }
        return $newlang;
    }
    ...
    function _convertToNameMode($lang, $convertKey = false)
    {
        ...
        if (is_string($lang)) {
            return ISO639::$method($lang);
        }

        $newlang = array();
        foreach ($lang as $key => $val) {
            if ($convertKey) {
                $newkey = ISO639::$method($key);
                $newlang[$newkey] = $val;
            } else {
                $newlang[$key] = ISO639::$method($val);
            }
        }
        return $newlang;
    }
